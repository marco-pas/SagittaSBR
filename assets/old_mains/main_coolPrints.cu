...
__global__ void create_world(hitable** d_list, hitable** d_world) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        d_list[0] = new sphere(vec3(0.0f, 0.0f, -4.0f), 0.5f);
...
for the RCS caluclation depending on phi and theta:

1) MONOSTATIC: you move both receiver and transmitter
    -> redo hit calculation
    -> redo PO integration

2) BISTATIC
    -> keep same hit calculation
    -> redo PO integration, but you have to consider there are object in between... 
        need of "observation shadowing" -> "visibility function"
